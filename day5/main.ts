...
function meetAt(year?: number, month?: number, day?: number): string {
  if (year && month && day) {
...
// [100,200,3,0,2,1]
let arr: number[] = [];
function findSmallestElement(arr: number[]) {
  if (arr.length === 0) {
    return 0;
  }
